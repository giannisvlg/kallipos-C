...
--8<-- "src/ch1_p1.c"
```
Ο κώδικας γράφεται σε έναν επεξεργαστή κειμένου, ως ένα αρχείο απλού κειμένου. Οδηγίες για τη ρύθμιση ενός περιβάλλοντος ανάπτυξης προγραμμάτων στην C υπάρχουν στο Παράρτημα Α. Ο κώδικας περιέχει τη συνάρτηση main(), που είναι το σημείο έναρξης εκτέλεσης για προγράμματα στη C. Στο συγκεκριμένο παράδειγμα η συνάρτηση main() περιέχει μια μόνο εντολή κλήσης της συνάρτησης printf(), που εκτυπώνει το κείμενο Hello World! και πραγματοποιεί αλλαγή γραμμής, λόγω της ύπαρξης του \n στο τέλος του κειμένου εντός των διπλών εισαγωγικών. Στη συνέχεια το πρόγραμμα τερματίζει τη λειτουργία του. Για να μπορεί να εκτελεστεί, θα πρέπει να προηγηθεί η διαδικασία μεταγλώττισης και σύνδεσης του κώδικα. Αυτό γίνεται με την ακόλουθη εντολή και, εφόσον δεν προκύψουν λάθη, δημιουργείται το εκτελέσιμο αρχείο. Το όνομα του εκτελέσιμου θα είναι a.out σε συστήματα Linux ή MacOS και a.exe σε συστήματα Windows.
...
Το εκτελέσιμο εκτελείται με απλή παράθεση του ονόματός του στη γραμμή εντολών. Σε συστήματα Linux ή MacOS πρέπει να προηγηθεί του ονόματος το ./, που υποδηλώνει τον τρέχοντα φάκελο, έτσι ώστε να εντοπιστεί η σωστή διαδρομή προς το αρχείο.
...
Hello World!
```
Αν υπάρχει συντακτικό σφάλμα στον κώδικα, τότε ο μεταγλωττιστής θα εμφανίσει μήνυμα σφάλματος και δεν θα δημιουργήσει το εκτελέσιμο. Στο ακόλουθο παράδειγμα (κώδικας 1.2) έχει σκόπιμα παραλειφθεί το δεξί διπλό εισαγωγικό στην printf(), με αποτέλεσμα να υπάρχει σφάλμα στον κώδικα.

```{.c title="ch1_p1_error.c" linenums="1"}
--8<-- "src/ch1_p1_error.c"
```
Η μεταγλώττιση θα δώσει τα ακόλουθα μηνύματα λάθους που περιγράφουν με ακρίβεια τόσο το σφάλμα
(warning: missing terminating " character), όσο και τη θέση του στο πρόγραμμα. Ωστόσο, σε
άλλες περιπτώσεις τα μηνύματα λάθους που παράγει ο μεταγλωττιστής μπορεί να μην είναι τόσο σαφή.
